...
// db.collection('cars').doc('lambo').update({model: 'Diablo', color: 'hot pink'})



//Get a single document (car) when we know the ID
db.collection('cars').doc('lambo').get()//.get returns doc. can also be used to return collection.
.then(doc => {
    console.log(doc.id)
...
db.collection('cars').where('year', '>=', 2015)
